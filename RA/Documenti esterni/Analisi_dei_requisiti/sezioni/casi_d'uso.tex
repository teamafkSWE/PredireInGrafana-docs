...
\label{par:UC8}
	...
	\begin{itemize}
		...
		\label{par:UC8.5}
	\subsection{UC8.5 - Disabilitazione salvataggio}
		\begin{itemize}
...
\label{par:UC8.6}
	\subsection{UC8.6 - Visualizzazione messaggio di notifica "Salvataggio interrotto"}
	\begin{itemize}
...
%REMOVE%
	%%\label{par:UC13}
	%\subsection{UC13 - Visualizzazione messaggio di alert "File JSON già caricato"}
		%\begin{itemize}
			%\item\textbf{Codice Identificativo}: UC13;
			...
			%\item\textbf{Postcondizioni}: l'utente ha visualizzato un messaggio di alert in cui viene segnalato che è già presente un file JSON caricato nel plugin;
			%\item\textbf{Scenario principale}:
			%	\begin{enumerate}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%	
	%\label{par:UC13}
	%\subsection{UC13 - Visualizzazione messaggio d'errore "Caricamento file JSON" }
	%	\begin{itemize}
	%		\item\textbf{Codice Identificativo}: UC13;
	%		\item\textbf{Titolo}: Visualizzazione messaggio d'errore "Caricamento file JSON";
	%		\item\textbf{Attore primario}: utente generico;
	%		\item\textbf{Descrizione}: attività in cui viene notificato che il file JSON che si sta cercando di caricare nel plugin non è adatto per effettuare previsioni;
	...
	%		\item\textbf{Postcondizioni}: l'utente ha visualizzato un messaggio d'errore in cui viene segnalato che la struttura del file selezionato non supporta una Support Vector Machine o Regressione lineare, di conseguenza viene riportato alla selezione del file JSON;
	%		\item\textbf{Scenario principale}:
	%			\begin{enumerate}
	...
	%				\item l'utente clicca il pulsante "X" per proseguire e viene riportato alla sezione di selezione.
	%			\end{enumerate}
	%	\end{itemize}	
