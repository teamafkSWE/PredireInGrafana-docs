...
% - Casi d'uso: Principali - %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	...
	\end{figure}	
		...
		\begin{itemize}
			...
			\item\textbf{Scenario principale}:
				\begin{enumerate}
					...
					\item (\hyperref[par:UC1.5]{UC1.5}) l'utente riceve in output il file JSON contenente i predittori per SVM/RL che viene salvato localmente.
				\end{enumerate}
			\item\textbf{Estensioni}: \hyperref[par:UC9]{UC9} estende \hyperref[par:UC1.3]{UC1.3}: viene visualizzato un messaggio d'errore se il file di addestramento è incompatibile con l'algoritmo scelto.
		...
		\begin{itemize}
			...
			\item\textbf{Scenario principale}: l'utente clicca la Combo Box con etichetta "Seleziona Algoritmo" e sceglie l'algoritmo (Support Vector Machine o Regressione Lineare).
		\end{itemize}
	...
	\label{par:UC1.4}
	\subsection{UC1.4 - Visualizzazione messaggio di notifica "Addestramento avvenuto con successo"}
		\begin{itemize}
			\item\textbf{Attore primario}: utente generico;
			\item\textbf{Precondizioni}: 
				\begin{enumerate}
	...
	\subsubsection{UC1.5 - Salvataggio file JSON}
		\begin{itemize}
			...
			\item\textbf{Scenario principale}: l'utente scarica il file JSON che è stato prodotto cliccando il pulsante di download. 		
		\end{itemize}
...
%Possibilità di inserire Scollegamento e modifica dentro a unico UC - "Operazioni sui collegamenti"
	...
	\label{par:UC4}
	\subsection{UC4 - Scollegamento del predittore}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	...
	\item\textbf{Postcondizioni}:
			...
			\item\textbf{Scenario principale}:
				\begin{enumerate}
					...
					\item (\hyperref[par:UC6.3]{UC6.3}) viene visualizzato un messaggio di notifica di "Monitoraggio avviato con successo";
					\item (\hyperref[par:UC6.4]{UC6.4}) una volta avviato il monitoraggio è possibile interromperlo cliccando il pulsante di interruzione.
					\item (\hyperref[par:UC6.5]{UC6.5}) viene visualizzata una notifica di "Monitoraggio interrotto";		
					\item (\hyperref[par:UC6.6]{UC6.6}) l'utente decide se salvare i dati di previsione cliccando il pulsante di salvataggio.
					\item (\hyperref[par:UC6.7]{UC6.7}) viene visualizzata un messaggio di notifica di "Invio dati avvenuto con successo";
				\end{enumerate}
			\item\textbf{Estensioni}:
				\begin{enumerate}
	...
	\subsubsection{UC6.2 - Avvio monitoraggio}
		...
		\label{par:UC6.3}
	\subsection{UC6.3 - Visualizzazione messaggio di notifica "Monitoraggio avviato con successo"}	
		\begin{itemize}
			\item\textbf{Attore primario}: utente generico;
		...
		\label{par:UC6.4}
	\subsection{UC6.4 - Interruzione monitoraggio}
	...
	\label{par:UC6.5}
	\subsection{UC6.5 - Visualizzazione messaggio di notifica "Monitoraggio interrotto"}
		\begin{itemize}
			\item\textbf{Attore primario}: utente generico;
	...
	\label{par:UC6.6}
	\subsubsection{UC6.6 - Salvataggio previsione}
		\begin{itemize}
			\item\textbf{Attore primario}: utente generico;
			\item\textbf{Precondizioni}:
				\begin{enumerate}
					\item è stata configurata correttamente la connessione al server tramite Grafana;
					\item l'utente ha avviato il monitoraggio (\hyperref[par:UC6.2]{UC6.2}) e dispone delle previsioni sul flusso di dati in quanto il calcolo di previsione è stato completato correttamente (\hyperref[par:UC6.3]{UC6.3}).
				\end{enumerate}
	...
	\label{par:UC6.7}
	\subsection{UC6.7 - Visualizzazione messaggio di notifica "Salvataggio previsione avvenuta con successo"}	\begin{itemize}
			\item\textbf{Attore primario}: utente generico;
			\item\textbf{Precondizioni}: l'utente ha cliccato il pulsante di invio previsioni (\hyperref[par:UC6.6]{UC6.6});
			\item\textbf{Postcondizioni}: l'utente ha visualizzato la notifica di "Salvataggio dati di previsione avvenuto con successo";
			\item\textbf{Scenario principale}:
				\begin{enumerate}
					\item l'utente visualizza il messaggio di notifica "Salvataggio previsione avvenuto con successo" in cui viene notificato che i dati di previsione sono stati inviati al Database (\hyperref[par:UC6.6]{UC6.6});
					\item l'utente clicca il pulsante "Conferma" per proseguire.		
				\end{enumerate}		
		\end{itemize}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	...
	\end{figure}
		...
		\begin{itemize}
			...
			\item\textbf{Scenario principale}: l'utente visualizza le previsioni calcolate. Il sistema si aggiorna costantemente grazie ai dati provenienti dal database, effettuando l'operazione di calcolo delle previsioni sul flusso dati associato ai predittori (\hyperref[par:UC6]{UC6}). Tale operazione di calcolo di previsione viene eseguito in base alla politica temporale stabilita in \hyperref[par:UC6.1]{UC6.1};		
